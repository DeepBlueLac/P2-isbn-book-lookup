...
- 书名、作者和 ISBN 搜索
- 只显示可直接下载的公版书
- Google Books 官方 EPUB/PDF 下载链接
- 非公版图书的在线预览和购买入口
- 封面、作者、出版社、日期、页数和简介
- 复制书目信息

网站不会把受限预览伪装成免费下载。只有 Google Books 将条目标记为 `publicDomain` 且提供 `downloadLink` 时，界面才显示下载按钮。
...
- 元数据、预览、购买和公版下载链接来自 [Google Books API](https://developers.google.com/books/docs/v1/using)。
- 项目不包含用户账户或数据库。
